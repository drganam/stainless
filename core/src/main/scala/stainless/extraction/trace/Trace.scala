...
trait Trace extends CachingPhase with IdentityFunctions with IdentitySorts { self =>
  ...
  def inductPattern(symbols: s.Symbols, model: FunDef, lemma: FunDef) = {
    ...
    //TODO try to merge with the other specializer
    val specializer1 = new Specializer(indPattern, indPattern.id, (lemma.tparams zip fi.tps).map { case (tparam, targ) => tparam.tp.id -> targ } .toMap, Map())
    ...
    val postLemma = speccedLemma.getSpec(PostconditionKind).map(post => 
      specializer1.transform(exprOps.replaceFromSymbols(specsMap, post.expr)))
