...
//      println("============================")
    ...
    override def implies(expr: Expr): Boolean = ocbsl.implies(conditions, ocbsl.codeOf(expr)(using exprCode))
  }
...
//          code2sig(cB)
    ...
    def codeOfIntLit(lit: BigInt, tpe: Type)(using Map[Variable, Code]): Code = codeOf(intLitOfType(lit, tpe))
